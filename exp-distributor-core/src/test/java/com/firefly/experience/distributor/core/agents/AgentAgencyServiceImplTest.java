...
class AgentAgencyServiceImplTest {
    ...
    void listAssignments_shouldReturnEmptyFluxBecauseSdkHasNoBody() {
        ...
        when(agentAgencyApi.listAgentAgencies(eq(distributorId), any())).thenReturn(Mono.empty());

        StepVerifier.create(service.listAssignments(distributorId))
                .verifyComplete();

        verify(agentAgencyApi).listAgentAgencies(eq(distributorId), any());
    }
    ...
    void assignAgentToAgency_shouldMapRequestAndBuildDtoFromReturnedId() {
        ...
        when(agentAgencyApi.assignAgentAgency(eq(distributorId), eq(command), any())).thenReturn(Mono.just(relationshipId));
        ...
        verify(agentAgencyApi).assignAgentAgency(eq(distributorId), eq(command), any());
    }
    ...
    void unassignAgent_shouldDelegateToApiAndComplete() {
        ...
        when(agentAgencyApi.removeAgentAgency(eq(distributorId), eq(relationshipId), any())).thenReturn(Mono.empty());

        StepVerifier.create(service.unassignAgent(distributorId, relationshipId))
                .verifyComplete();

        verify(agentAgencyApi).removeAgentAgency(eq(distributorId), eq(relationshipId), any());
    }
}
